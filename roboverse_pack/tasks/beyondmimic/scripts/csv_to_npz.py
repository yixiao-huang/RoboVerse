...
# Pre-defined configs
##
from roboverse_pack.tasks.beyondmimic.isaaclab.robots.g1 import G1_CYLINDER_CFG
...
class MotionLoader:
    """Loads and processes motion data from a csv file."""
    ...
    def _load_motion(self):
        ...
        motion = motion.to(torch.float32).to(
            self.device
        )  # [n_frames, 3 (base position) + 4 (base rotation) + n_dofs (each DoF corresponds to one changing value)]
        self.motion_base_poss_input = motion[:, :3]
        self.motion_base_rots_input = motion[:, 3:7]
        self.motion_base_rots_input = self.motion_base_rots_input[:, [3, 0, 1, 2]]  # convert to wxyz
        self.motion_dof_poss_input = motion[:, 7:]  # [n_frames, n_dofs]
    ...
    def _compute_velocities(self):
        ...
        self.motion_base_lin_vels = torch.gradient(self.motion_base_poss, spacing=self.output_dt, dim=0)[
            0
        ]  # [n_frames_out, 3]
        self.motion_dof_vels = torch.gradient(self.motion_dof_poss, spacing=self.output_dt, dim=0)[
            0
        ]  # [n_frames_out, n_dofs]
        self.motion_base_ang_vels = self._so3_derivative(self.motion_base_rots, self.output_dt)  # [n_frames_out, 3]
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
    ...
    robot = scene["robot"]
    robot_joint_indexes = robot.find_joints(joint_names, preserve_order=True)[
        0
    ]  # Get the corresponding joint's index for a given joint name
