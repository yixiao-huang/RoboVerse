...
class TrackingBaseEnv:
    ...
    * **Recorder Manager**: The recorder manager that handles recording data produced during different steps
    ...
    def __init__(self, cfg: ManagerBasedEnvCfg):
        ...
        log.info("[INFO]: Base environment:")
        log.info(f"\tEnvironment device    : {self.device}")
        log.info(f"\tEnvironment seed      : {self.cfg.seed}")
        log.info(f"\tPhysics step-size     : {self.physics_dt}")
        log.info(f"\tRendering step-size   : {self.physics_dt * self.cfg.sim.render_interval}")
        log.info(f"\tEnvironment step-size : {self.step_dt}")
        ...
        log.info("[INFO]: Scene manager: ", self.scene)
        ...
        if builtins.ISAAC_LAUNCHED_FROM_TERMINAL is False:
            log.info("[INFO]: Starting the simulation. This may take a few seconds. Please wait...")
    ...
    def load_managers(self):
        ...
        log.info("[INFO] Event Manager: ", self.event_manager)
        # -- recorder manager
        self.recorder_manager = RecorderManager(self.cfg.recorders, self)
        log.info("[INFO] Recorder Manager: ", self.recorder_manager)
        # -- action manager
        self.action_manager = ActionManager(self.cfg.actions, self)
        log.info("[INFO] Action Manager: ", self.action_manager)
        # -- observation manager
        self.observation_manager = ObservationManager(self.cfg.observations, self)
        log.info("[INFO] Observation Manager:", self.observation_manager)
        ...
        if self.__class__ == TrackingBaseEnv and "startup" in self.event_manager.available_modes:
