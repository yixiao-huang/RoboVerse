...
G1_CYLINDER_CFG = ArticulationCfg(
    ...
    # NOTE `init_state` corresponds to `ArticulationData.default_joint_pos`
...
for a in G1_CYLINDER_CFG.actuators.values():
    ...
    if not isinstance(e, dict):  # if `e` is a scalar, all joints specified by `names` will have the same value
        e = {n: e for n in names}
    if not isinstance(s, dict):  # if `s` is a scalar, all joints specified by `names` will have the same value
