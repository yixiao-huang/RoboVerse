"""Handler wrapper for automatic visualization updates.

This module provides a wrapper for BaseSimHandler that automatically updates
Rerun and Viser visualizations when set_states or simulate is called.
"""
...
class VizHandlerWrapper:
    """Wrapper for BaseSimHandler that automatically updates visualization.

    This wrapper intercepts set_states and simulate calls to automatically
    update Rerun and/or Viser visualizations. This allows visualization to
    work at the handler level without needing to wrap the environment.

    Args:
        handler: BaseSimHandler instance to wrap
        ...
        rerun_app_name: Application name for Rerun (default: "RoboVerse Handler")
        rerun_spawn: Whether to spawn Rerun viewer automatically (default: True)
        viser_port: Port for Viser server (default: 8080)
        update_freq: Update visualization every N calls to reduce resource usage (default: 1)
    """
    ...
    def __init__(
        ...
        rerun_app_name: str = "RoboVerse Handler",
    ...
    def _setup_rerun(self, app_name: str, spawn: bool):
        ...
        if not RERUN_AVAILABLE:
            logger.warning("Rerun requested but rerun-sdk not available. Install with: pip install rerun-sdk")
            return
        ...
        try:
            ...
            # Get initial states and visualize
            ...
            logger.info("Rerun visualization initialized for handler")
    ...
    def _setup_viser(self, port: int):
        ...
        if not VISER_AVAILABLE:
            logger.warning("Viser requested but viser not available. Install with: pip install viser")
            return
        ...
        try:
            ...
            if hasattr(obs, "objects") and hasattr(obs, "robots"):
                ...
                if self.handler.objects:
                    self.viser_visualizer.visualize_scenario_items(self.handler.objects, default_object_states)

                if self.handler.robots:
                    self.viser_visualizer.visualize_scenario_items(self.handler.robots, default_robot_states)
            ...
            logger.info("Viser visualization initialized for handler")
        ...
        try:
            ...
            if self.rerun_visualizer is not None:
                ...
                if env_states and len(env_states) > 0:
                    ...
                    # Update time step
                    self.rerun_visualizer.set_time(self.call_count)
    ...
    def set_states(self, states: TensorState | list[DictEnvState], env_ids: list[int] | None = None) -> None:
        """Set states and update visualization."""
        self.handler.set_states(states, env_ids)
    ...
    def __getattr__(self, name: str):
        """Proxy all other attributes to the wrapped handler."""
